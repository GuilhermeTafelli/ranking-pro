...
const {mapListToResponse, mapCompleteListToResponse} = require("./mappers/OrderServiceMapper")
class OrderService{
    ...
    async getComplitedById(orderId){

        try {        
            const orders = await Order.findOne({where: {id: orderId}, include: {model:User}})
            return mapCompleteListToResponse(orders)
        }
        catch(error){
            console.log(error)
